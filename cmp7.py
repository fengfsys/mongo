...
def compare_raw_lines(doc1, doc2):
    flattened_doc1 = flatten_json(doc1)
    flattened_doc2 = flatten_json(doc2)
    
    lines1 = set(f"{key}: {value}" for key, value in flattened_doc1.items())
    lines2 = set(f"{key}: {value}" for key, value in flattened_doc2.items())
